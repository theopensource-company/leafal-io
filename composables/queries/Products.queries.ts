...
export const useProduct = async (slug: string) => new Promise((resolve, reject) => {
    useDatabase()
        .then(async c => resolve((await c.query<[Product[]]>('SELECT * FROM product WHERE slug = $slug', { slug }))[0][0]))
        .catch(reject)
})
